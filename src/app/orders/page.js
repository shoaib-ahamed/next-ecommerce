...
import { getAllOrdersForUser } from "@/controller/order";
          ...
          <div>
            <div className="px-4 py-6 sm:px-8 sm:py-10">
              <div className="flow-root text-black">
                {allOrdersForUser && allOrdersForUser.length ? (
                  <ul className="flex flex-col gap-4">
                    {allOrdersForUser.map((item) => (
                      <li
                        key={item._id}
                        className="bg-gray-200 text-black shadow p-5 flex flex-col space-y-3 py-6 text-left"
                      >
                        <div className="flex">
                ...
                ) : <div className="h-[70vh] w-full flex justify-center items-center"> There is no order! please do some... </div>}
              </div>
            </div>
          </div>
        </div>
